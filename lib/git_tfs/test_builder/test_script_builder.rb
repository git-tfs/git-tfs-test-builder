...
<%= render_each changeset_template, :changeset, sorted_changesets %>
        ...
        private static byte[] Read(string itemContentHash)
        {
            ...
            var fixtureResourceName = "Sep.Git.Tfs.Test.Fixtures.<%= class_name %>." + itemContentHash;
            using (var stream = typeof(<%= class_name %>).Assembly.GetManifestResourceStream(fixtureResourceName))
            {
                if (stream == null)
                    throw new Exception("Could not find resource " + fixtureResourceName);

                using (var reader = new GZipReader(stream, CompressionMode.Decompress))
                {
...
END_TEMPLATE
      ...
      def change_template
        <<END_TEMPLATE
                .Change(<%=enum "TfsChangeType", change.change_types %>, <%= enum "TfsItemType", change.item_type %>, <%=cs change.item_path %>, Read(<%=cs change.item_hex_hash %>), <%=cs change.item_id.to_i %>)
END_TEMPLATE
      end
